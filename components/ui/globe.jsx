...
import countries from "@/data/globe.json"; // Assuming the country data is in this file
...
export function Globe({ globeConfig, data }) {
  ...
  const defaultProps = {
    ...
    globeColor: "#1d072e", // Original globe color
    emissive: "#000000",
  ...
  const _buildMaterial = useCallback(() => {
    ...
    globeMaterial.color = new Color(globeConfig.globeColor); // Set globe color
    globeMaterial.emissive = new Color(globeConfig.emissive); // Emissive settings
  ...
  useEffect(() => {
    if (globeRef.current) {
      _buildData();
      _buildMaterial();
      // Add the polygons data (world map)
  ...
  }, [_buildData, _buildMaterial, defaultProps.polygonColor]);
...
export function WebGLRendererConfig() {
  ...
  useEffect(() => {
    ...
    gl.setClearColor(0x000000, 0); // Black background (previous design)
  }, [gl, size]);
